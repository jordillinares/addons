...
class stock_move(models.Model):
    ...
    # manually produce a smaller qty than the original move's, and without
    # copy=True, split move would be created, but not be shown as production
    # pending.
    production_id = fields.Many2one('mrp.production',
                                    'Production Order for Produced Products',
                                    select=True, copy=True)
    ...
    def action_consume(self, cr, uid, ids, product_qty, location_id=False,
        ...
        res = super(stock_move,
                    self
                    ).action_consume(cr, uid, ids, product_qty,
        ...
        for move in self.browse(cr, uid, ids, context=context):
            for (id, name) in product_obj.name_get(cr, uid,
                                                   [move.product_id.id]):
                if 'consume' in context and context.get('consume', False):
                    production_ids = production_obj.search(cr, uid,
                                                           [('move_lines',
                                                             'in',
                                                             [move.id])])
                    production_id = production_ids and \
                        production_ids[0] or False
                    if production_id:
                        production = production_obj.browse(cr, uid,
                                                           production_id,
                                                           context=context)
                        add_destination = ""
                        if production:
                        ...
                        for lot in move.lot_ids:
                            message = _("%s %s of lot %s of %s "
                                        "have been "
                                        "consumed.") % (move.product_qty,
                                                        move.product_uom.name,
                                                        lot.name, name)
                            if add_destination:
                                destination_list = []
                                if lot.destination:
                                    destination_list += lot.destination.split(
                                        ";")
                                if add_destination not in destination_list:
                                    destination_list.append(add_destination)
                                destination = ";".join(destination_list)
                                lot_obj.write(cr, uid,
                                              [lot.id],
                                              {'destination': destination, })
                            else:
                                message = _("%s %s of %s have "
                                            "been consumed."
                                            ) % (move.product_qty,
                                                 move.product_uom.name,
                                                 name)
                            self.pool.get('mrp.production'
                                          ).message_post(cr, uid,
                                                         [production_id],
                                                         message,
                                                         _('Raw material '
                    ...
                    if production_id:
                        ...
                        for lot in move.lot_ids:
                            message = _("%s %s of lot %s of %s have been "
                                        "produced.") % (move.product_qty,
                                                        move.product_uom.name,
                                                        lot.name, name)
                            if add_origin:
                                origin_list = []
                                if lot.origin:
                                    origin_list += lot.origin.split(";")
                                if add_origin not in origin_list:
                                    origin_list.append(add_origin)
                                origin = ";".join(origin_list)
                                lot_obj.write(cr, uid,
                                              [lot.id],
                                              {'origin': origin})
                            else:
                                message = _("%s %s of %s have been "
                                            "produced."
                                            ) % (move.product_qty,
                                                 move.product_uom.name,
                                                 name)
                            self.pool.get('mrp.production'
                                                         ...
                                                         _('Manufactured '
                                                           'product'),
                                                         context=context)
                            # In our manual consume/produce version, we didn't
                            # use 'consumed_for' until here. But here we'll
                            # assign all already consumed materials that do
                            # not have a value in 'consumed_for' to this
                            # production move.
                            consume_parent_move_ids = [x.id
                                                       for x in
                                                       production.move_lines2
                                                       if not x.consumed_for]
                            self.write(cr, uid, consume_parent_move_ids,
                                       {'consumed_for': move.id},
                                       context=context)
        return res
...
class stock_production_lot(models.Model):
    ...
    def init(self, cr):
        """
        Create pl/pgqsl functions for upstream and downstream
        lot traceability on module installation"
        """
        ...
        # Check if custom SQL type 'prodlot_hierarchy' is present in the DB.
        # If not, create it. if it exists, delete functions, delete type and
        # recreate all (it's unnecessary on a production environment, but
        # very useful while adjusting SQL functions).
    ...
    def _child_compute(self):
        for record in self:
            ...
            child_ids = [x[0] for x in data
                         if not x[0] in [y.id for y in self]]
    ...
    def _parent_compute(self):
        for record in self:
            ...
            """ % (record.id, record.id))
            data = self.env.cr.fetchall()
            parent_ids = [x[0] for x in data
                          if not x[0] in [y.id for y in self]]
            record.parent_complete_ids = self.browse(parent_ids)

    child_complete_ids = fields.Many2many(comodel_name='stock.production.lot',
                                          string="Lot hierarchy upstream",
                                          compute=_child_compute)
    parent_complete_ids = fields.Many2many(comodel_name='stock.production.lot',
                                           string="Lot hierarchy upstream",
                                           compute=_parent_compute)

    @api.multi
    def from_hierarchy_open_lot_form(self):
        # Action called from the server action linked to the
        # tree_but_open in the lots hierarchical view
        return {
