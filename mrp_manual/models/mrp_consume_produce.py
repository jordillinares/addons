...
class stock_move_consume(models.TransientModel):
    ...
    def onchange_product_id(self):
        ...
        self.removal_strategy = self.location_id.get_removal_strategy(
            self.location_id, self.product_id).upper()
        if move.reserved_quant_ids:
            ...
            # Please note 'ordered_quants_info' variable includes not only
            # reserved quants for the movement, but also all quants of the
            # product that are not reserved and have a lot_id, thus being able
            # to be selected here.
            ordered_quants_info = self.env['stock.quant'].quants_get(
                move.location_id,
                move.product_id,
                suggested_quant.qty,
                domain=['&', '|', ('reservation_id', '=', move.id),
    ...
    def onchange_lot_id(self):
        ...
        if self.restrict_lot_id:
            max_qty = move.product_id.with_context(
                location=move.location_id.id,
                lot_id=self.restrict_lot_id.id).qty_available
    ...
    def onchange_product_qty(self):
        ...
        if self.restrict_lot_id:
            ...
            if self.product_qty > max_qty:
                warning = {
                    'title': _('Warning!'),
                    'message': _('You are trying to consume %s %s of lot %s, '
                                 'but there are only available %s %s on %s.\n'
                                 'Quantity automatically set to maximum '
                                 'available.')
                    % (self.product_qty,
                       move.product_id.uom_id.name,
                       self.restrict_lot_id.name,
                       max_qty,
                       move.product_id.uom_id.name,
...
class stock_move_produce(models.TransientModel):
    ...
    location_dest_id = fields.Many2one(
        'stock.location', 'Destination loc.', required=True)
    lot_id = fields.Many2one('stock.production.lot', 'Lot')

    # TOFIX: product_uom should not be different from product's default uom.
    # Qty should be converted to UOM of original move line before being
    # produced
    def default_get(self, cr, uid, fields, context=None):
        if context is None:
            context = {}
        res = super(stock_move_produce, self).default_get(
    ...
    def quick_lot_create(self):
        for detail in self:
            if detail.product_id and \
                detail.product_id.lot_creation_mode == 'auto' and \
                (detail.product_id.track_production or
                 detail.product_id.track_all):
                detail.lot_id = self.env['stock.production.lot'].with_context(
                    product_id=detail.product_id.id).create({})
            else:
                raise except_orm(_('Warning!'),
                                 _('Product lot tracking is not enabled, or'
                                   'lot creation mode is \'manual\'\n. A new '
                                   'lot number won\'t be automatically '
                                   'created.'))
        if self and self[0]:
    ...
    def do_move_produce(self):
        ...
        for record in self:
            if move_ids and move_ids[0]:
                ...
                # Yes, we create a wizard for production very similar to
                # consume wizard, but at the end of both wizards we use
                # the same 'action_consume' method.
                move.action_consume(
                    record.product_qty,
                    record.location_id.id,
                    restrict_lot_id=record.lot_id.id)
